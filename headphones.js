/**
 * Boost Electronics – Headphones Page
 * Final stable version – modal now sticks perfectly.
 */
...
document.querySelectorAll('.categories-grid a, .dropdown-menu a, .mobile-dropdown-menu a').forEach(link => {
  link.addEventListener('click', (e) => {
    ...
    if (brand) {
      ...
      const filtered = headphones.filter(p => p.category.toLowerCase() === category.toLowerCase() || p.brand === category);
      renderHeadphones(filtered);
    }
    categoriesDropdown.classList.remove('show');
  });
...
function openModal(productId) {
  ...
  const related = headphones.filter(p => p.id != productId && p.category === product.category).slice(0, 6);
  ...
  // Show modal – using flex (already in CSS)
  ...
  setTimeout(() => {
    ...
    // Force a reflow to prevent any movement – container is already static
    modalSwiper.update();
  }, 100);
...
document.addEventListener('click', (e) => {
  if (e.target.closest('.add-to-cart-card')) {
    ...
    if (card) {
      ...
      const product = headphones.find(p => p.id == id);
      addToCart(product, 1);
    }
  }
});
...
document.querySelector('.whatsapp-float').addEventListener('click', (e) => {
  e.preventDefault();
  const message = "Hello, I'm interested in your electronics products";
